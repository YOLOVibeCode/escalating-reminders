...
describe('envFromHost — EscalatingReminders hostname rules', () => {
  it('maps the friendly domains to their environment', () => {
    expect(envFromHost('dev.escalating-reminders.com', HOST_RULES)).toBe('dev');
    expect(envFromHost('staging.escalating-reminders.com', HOST_RULES)).toBe('uat');
    expect(envFromHost('uat.escalating-reminders.com', HOST_RULES)).toBe('uat');
    expect(envFromHost('escalating-reminders.com', HOST_RULES)).toBe('production');
    expect(envFromHost('www.escalating-reminders.com', HOST_RULES)).toBe('production');
  });
  ...
  it('never mislabels the bare prod domain as a sub-env (no false banner on prod)', () => {
    expect(envFromHost('escalating-reminders.com', HOST_RULES)).not.toBe('dev');
    expect(envFromHost('escalating-reminders.com', HOST_RULES)).not.toBe('uat');
    expect(envFromHost('www.escalating-reminders.com', HOST_RULES)).not.toBe('dev');
    expect(envFromHost('www.escalating-reminders.com', HOST_RULES)).not.toBe('uat');
  });
});
